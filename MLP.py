...
def Multi_layer_net(inputs, target, active, params):
    #inputs : (1:N)
    #W1 : (N:D) b1 : (D)
    #W2 : (D:1) b2 : (1)
    #active : n
...
def MLP_trainer(input_seq, target_seq, active_seq, dimention, learning_rate, decay, epoch):
    option_size = 7
    params = {}
    ...
    for e in range(epoch):
        for i in range(len(input_seq)):
            loss, grad = Multi_layer_net(input_seq[i], target_seq[i], active_seq[i], params)
...
def main():
    ...
    print('3. training...')
    params = MLP_trainer(input_seq, target_seq, active_seq, dimention=64, learning_rate=0.0001, decay=0.98, epoch=10)
...
def test():
    ...
    for i in range(10):
        ...
        print("real : %f, predict : %f" %(target_seq[i], Multi_layer_net(input_seq[i], None, active_seq[i], params)))
